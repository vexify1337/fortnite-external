...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    // Calculate the normalized value of p_data within the range [p_min, p_max]
    float normalized_value = 0.0f;
    if (data_type == ImGuiDataType_Float) {
        float v = *(float*)p_data;
        float min = *(float*)p_min;
        float max = *(float*)p_max;
        normalized_value = (v - min) / (max - min); // Normalize to [0, 1]
    }
    
    // Add support for other data types as needed

    // Clamp the normalized value to avoid any overflow
    normalized_value = ImClamp(normalized_value, 0.0f, 1.0f);

    // Use the normalized value to calculate the grab position
    static std::unordered_map<ImGuiID, float> values;
    auto value = values.find(id);

    if (value == values.end()) {
        values.insert({ id, normalized_value });
        value = values.find(id);
    }

    // Animate the grab position based on the normalized value
    value->second = ImLerp(value->second, normalized_value, ui::anim_speed * 3.0f);

    // Render the slider
    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), 2);
    window->DrawList->AddRectFilled(bb.Min, { bb.Min.x + value->second * (bb.Max.x - bb.Min.x), bb.Max.y }, GetColorU32(ImGuiCol_Scheme), 2);
    window->DrawList->AddCircleFilled({ bb.Min.x + value->second * (bb.Max.x - bb.Min.x), bb.GetCenter().y }, 7, GetColorU32(ImGuiCol_SliderGrab));
